export default function Model(gl,shProgram) {
    ...
    this.CreateSurfaceData = function() {
        ...
        let tangents = []; // To store vertex normals
        let uvs = [];
        ...
        for (let i = 0; i <= uSegments; i++) {
            let u = -Math.PI + (2 * Math.PI * i) / uSegments; // Range: [-Pi : Pi]

            let x_u = a * Math.pow(Math.cos(u), 3);
            let z_u = a * Math.pow(Math.sin(u), 3);

            for (let j = 0; j <= vSegments; j++) {
                let v = ( 2 * Math.PI * j) / vSegments; // Range: [0, 2 * Pi]
                ...
                vertices.push(X, Y, Z);
                uvs.push(((u / Math.PI) + 1.0) / 2.0, v / (2.0 * Math.PI));
            }
        }
